...
export default class SteadyFlowLayer {
    ...
    render(gl, matrix) {
        ...
        if(this.maskUrl) this.maskBinding.executable = true;
    ...
    async init() {
        ...
        let getVoronoi = async (url) => {
            ...
            const res = await axios.get(url, { responseType: 'arraybuffer' })
            console.log(new Float32Array(res.data));
        ...
        if (this.maskUrl) {
            ...
            this.voronoiPipeline = scr.renderPipeline({
                name: 'Render Pipeline (Voronoi Flow)',
                shader: { module: scr.shaderLoader.load('Shader (Flow Voronoi)', '/shaders/flowVoronoi.wgsl') },
            })
        }
        ...
        if (this.maskPass) this.map.add2PreProcess(this.maskPass)
