...
const init = async (sceneTexture) => {
    ...
    const transHighTex = Scratch.imageLoader.load('transHighTex', parser.transform2DHighResource, undefined, 'rg32float')
    const transLowTex = Scratch.imageLoader.load('transLowTex', parser.transform2DLowResource, undefined, 'rg32float')

    const seedTexture = Scratch.imageLoader.load('seedTexture', parser.seedingResourceArray[0])
    ...
    simulation_pipeline = Scratch.ComputePipeline.create({
        ...
        shader: Scratch.shaderLoader.load('FFsimu', '/shaders/FFsimu.wgsl')
    })
...
let stopped = false;
const tickLogic = (mapbox_matrix, mercatorCenter) => {
    ...
    g_matrix = new Float32Array(mapbox_matrix)
