...
const init = async (sceneTexture) => {
        ...
        blocks: [
            Scratch.bRef({
                ...
                map: {
                    ...
                    speedFactor: {
                        type: 'f32',
                        value: () => controller.speedFactor * 20.0,
                    },
                    randomSeed: {
                        type: 'f32',
                        value: () => Math.random(),
    ...
    simulation_pipeline = Scratch.ComputePipeline.create({
        ...
        shader: Scratch.shaderLoader.load('FFsimu', '/shaders/FFsimuLHY.wgsl')
    })

    let render_pipeline = Scratch.RenderPipeline.create({
        name: 'render_pipeline',
        shader: { module: Scratch.shaderLoader.load('FFrendLHY', '/shaders/FFrendLHY.wgsl') },
