...
let parser
let rootPath = '/api/v1/data/nc/field/wind/pic?name='
let controller
let frameTimer
...
const init = async (sceneTexture) => {
    ...
    parser = new DescriptionParser("/api/v1/data/nc/field/wind/json?name=new_windfield.json")
    ...
    simulation_pipeline = Scratch.ComputePipeline.create({
        ...
        shader: Scratch.shaderLoader.load('FFsimu', '/shader/FFsimuLHY.wgsl')
    })

    let render_pipeline = Scratch.RenderPipeline.create({
        name: 'render_pipeline',
        shader: { module: Scratch.shaderLoader.load('FFrendLHY', '/shader/FFrendLHY.wgsl') },
