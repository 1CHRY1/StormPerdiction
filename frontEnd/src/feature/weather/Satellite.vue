...
onMounted(async () => {
  ...
  timeMap.value = time
  timeListArray.value = [...(time.get(options.value[1].value) as Set<string>)]
  selectType.value = options.value[1].value
...
<template>
  <div class="flex h-full">
    <div class="flex flex-auto justify-center items-center bg-zinc-200">
      <img class="flex-auto m-4 max-h-[90vh] object-contain" :src="imageUrl" />
    </div>
