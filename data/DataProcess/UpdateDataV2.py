...
def insert_Nottyphdata(db_path, name, time, hshice, hybresult, manual):
    # 插入无台风数据
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    updatetime = datetime.now()
    # 插入数据
    cursor.execute(f'''
            INSERT INTO {name} ( updatetime, time, iftyph, hshice, hybresult, manual )
            VALUES ('{updatetime}', '{time}', '0', '{hshice}', '{hybresult}', '{manual}')
        ''')
    conn.commit()
    conn.close()
            ...
            if (JudgeIfTyph(txtPath) == True):
                ...
                for file in files:
                    ...
                    if file.endswith(".mat"):
                        ...
                        mat_data = loadmat(mat_path)
                        try:
                            hshice = mat_data['hshice']
                            hybresult = mat_data['hybresult']
                            # 将预报数据存入数据库
                            insert_Nottyphdata(db_path_Forcasting, name, time, hshice, hybresult, manual)
                        except Exception as e:
                            print(e)
                    # 处理nc数据
                    if file.endswith(".nc"):
